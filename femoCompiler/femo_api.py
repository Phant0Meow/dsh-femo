...
2026-09-13 API 化改造：宿主桥（dshAdapter/python/femo_bridge.py，以及未来
任何宿主的桥/内嵌宿主）只准 import 本文件，不再直接触碰引擎内部模块
...
与 CLI 的关系：
  - 干跑已进本文件（debug_dry_run），但 dsh 宿主仍以一次性子进程跑 CLI
    （femoToolcall/femo_debugger.py run）：干跑要能被 terminate 不留僵尸、
    不与运行桥同进程（桥进程内禁用本函数——cmd_run/print_report 的 print
    会污染 NDJSON 事件流）。本函数面向内嵌宿主与测试进程内直用。
  - 台账直查（femoToolcall/chronica.py）不进本文件：它是裸 sqlite3 只读
    （WAL/immutable URI）+ 人读排版，不经任何引擎接口——展示层工具，
