...
import FEMOEditor from '../../../../femo2host/femo_gen_api'
...
function FemoEditorPage({ sessionId, injected }: { sessionId: string; injected: EditorPageInjected }): JSX.Element {
  ...
  const onRestoreError = useCallback((message: string): void => {
    ...
    }).catch((error: unknown) => { console.warn('[femo-plugin] editor-error 上报失败:', error) })
  }, [sessionId])
  ...
  const resolveConflictByReload = (): void => {
    ...
    void loadSessionState()
      .catch((error: unknown) => { console.warn('[femo-plugin] conflict reload failed:', error) })
  }
