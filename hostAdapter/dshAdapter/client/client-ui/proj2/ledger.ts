 ...
 *
 *   一轮的身份 = 宿主写下的开轮锚点 `femo-plugin/chat{kind:'live', turn, actor}`
...
/**
 * 事件 → 轮身份（唯一入口）。认得的形态：
 *  - `femo-plugin/chat{kind:'live', turn}` —— **开轮锚点**（start，每轮一条）
 *  - `femo-plugin/chat{kind:'speaker'|'prompt'|'error', turn}` —— 轮内归属行
