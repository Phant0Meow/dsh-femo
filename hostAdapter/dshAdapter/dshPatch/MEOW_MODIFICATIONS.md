...
### 发现（进 todo）
- deriveMessages 只投影 user/message、assistant/message、tool/result 三类事件 → `femo-plugin/chat`（flow_done/flow_error/flow_stopped 写入）不进主模型 LLM 上下文：persona 说"系统会告诉你"目前只对 UI 成立、对主模型不成立 → 已记 meow-memory todo「主模型收运行通知（persona 行为对齐）」
...
3. **agent-preset/selected 监听扩展**（原监听只写 presetOverrides）：preset 切到 femo-plugin 且未注入 → `ctx.agents.get(sessionId)` 拿 agent → `injectFemoDocs(agent.ctx)` 补注入（防重跳过）；切走 → dispose 清除 section 并删表（普通会话不留 Femo 文档段）。
### 坑
...
改 dsh-home/.agent-presets/femo-plugin/agent.cordis.yml persona 两段：
...
3. **src/index.ts**：新增 collectLlmModels（ctx.get('llm') 聚合 listProviders+listModels，llm 缺失/单 provider 失败兜底默认，绝不整体失败）；新路由 GET /femo-plugin/models（前端下拉数据源）；startRunOnSession run 参数带 models（引擎编译校验白名单）；runAiSubagent 新增 resolveSourceModel——source → agentOptions(provider, model)，空用配置默认
4. **femoGen/src/projectPanel.jsx**：导出 useModelList（fetch /femo-plugin/models）+ sourceOptions（空默认 + 全部 provider/model + 原值不在列表时追加防丢）；ai 类型 source 文本框 → 下拉（显示 provider/model），human 保留文本框（placeholder 改 数字ID）；新增 actor 默认 source 'deepseek' → ''（旧默认值裸 id 在 deepseek-official 下不存在，会触发编译错误）
...
用户需求：给 femo-plugin 加一个 AI 能用的工具，查看本 session 挂载的剧本内容。
...
- femoGen 右上角三态按钮（mobileView.jsx L270-296 → FemoWorAuto.jsx handlers）：
  ...
  - 「暂停」（running）→ POST /femo-plugin/pause → bridge pause（=stop 半实现）
...
1. **src/index.ts**：femo-plugin/event 回调里 flow_stopped 事件附加 `paused: runState.pausedByUser` 标记（broadcastSse + lastEvents 重放都携带）——暂停/停止由宿主判定，前端可见
...
1. **dsh-home/.agent-presets/femo-plugin/agent.cordis.yml**：persona 全文重写（`>-` 折叠 → `|-` 字面块保留模板换行）：新增【这是什么模式】（心智模型：actors/action/mainflow/scope/vars + "上下文不是变量"哲学）、【语法速查表】（14 行内联）、【关于 soul】（非必须；无 soul=裸执行者看全量上下文）、最小模板（Goal 模式裸 actor 版，无 session=new 无 soul）、【运行】双工作流（结果导向自主闭环 / 过程导向用户参与）+ 三工具教学；【会话模式】修正"剧本后台运行不被打扰、用户消息能收到"；保留用户定稿【剧本存哪】【运行出错时】
...
### 机制（在 dsh 源码验证可行）
主模型上下文 = `session.deriveMessages()`，只认 `user/message`/`assistant/message`/`tool/result` 三种 surface 事件（packages/core/session/src/surface.ts），`femo-plugin/chat` 永远进不了——此路不通。正确通道 = **systemPrompt.section**（packages/core/system-prompt）：section 的 `text` 支持函数，每次主模型调用组装 system prompt 时动态求值；system prompt 本身不进用户可见聊天 → 天然满足"注入上下文不污染界面"。空 text 的 section 被 renderPrompt 过滤。
...
- start-meow.bat 重启 3081：端口监听 ✓；GET / → 200；GET /femo-plugin/session-state → 200 正常 JSON；GET /femo-plugin/actors → 200 {ok:true,actors:[]}；meow-smooth 压缩代理 8444 随起 ✓
...
1. 删除 apply() 内 StreamPreviewBar 挂载块（独立 React 根 + portal 到 body 的 #femo-plugin-stream-preview）。
...
### 诊断（diag-uncaught.mjs 展开 AggregateError——Node 打印 AggregateError 不含 errors 数组，femo-plugin 的 uncaughtException handler 只打 stack 也看不到；脚本 prependListener 抢先展开）
两个失败 entry，**均与 femo-plugin 无关**（femo 路由+四工具注册成功、meow-smooth 正常）：
...
构建补齐后服务能起但出现新日志 `[femo-plugin] subprocess service unavailable; bridge not started`——插件 apply 完成早于 base bundle 的 subprocess provider（插件树并发装配完成顺序不定；rc.2 插件更多、固定 1s 延迟不再够）。修复：`bridge.start(ctx, config, attempt)` 改为轮询等待（subprocess 未就绪每秒重试，上限 30 次），就绪后打 `subprocess service ready after Ns wait`。
...
3. `.agent-presets/femo-plugin/agent.cordis.yml`（dsh-home）：系统提示词【运行】段 2 处——主模型人设文本的真正源头在此 preset，不在插件源码
...
### 修改明细
1. `src/engine-events.ts`：新增 steerMainAgent()（ctx.agents.get(sid) 或 ctx.get('agents') 拿主模型 agent，构造 {id,role:'user',content,source:{kind:'plugin',plugin:'femo-plugin'}} 后 steer）；flow_done/flow_error 两分支的 runNotices.set 替换为 steerMainAgent；flow_stopped 分支仅删除 runNotices.set（停止/暂停由发起方经工具返回值已知悉）；删除 flow_start 的 ensureRunNotice 调用
2. `src/persona.ts`：删除 femo:notify 三件套（runNotices/runNoticeSections/injectRunNotice/ensureRunNotice）及头注释更新
3. `src/tools.ts`：femo-run 描述改为"跑完/出错会有 [femo-plugin] 开头的插件消息直接发进对话流"
...
2. src/client-ui/editor-view.tsx：SSE 监听加 run_request 分支→editorRef.triggerRun('ai')；新增 reportRunResult → POST /femo-plugin/run-result（带 sessionId）。
...
5. src/routes.ts：新增 route POST /femo-plugin/run-result（resolve 对应 sessionId 等待者；无等待者静默忽略）。
### 生效条件
...
1. 新增 src/client-ui/editor-page.tsx：①模块级单页 store（target/锚点/会话引用计数）+ 全局控制 SSE（/femo-plugin/events 单例，管 run_request 与 script_changed）；②mountFemoEditorPage = body 级隐藏容器（fixed inset:0 + visibility:hidden，保留视口尺寸使编辑器布局计算一致）+ createRoot（react-dom/client 已确认在 seed 映射里）；③EditorPageRoot 订阅 store、applyPagePlacement 把根容器 DOM 挪进锚点/挪回隐藏位（同一个 DOM 节点，React 状态零丢失；锚点销毁时即使根容器被连带 detach，fibers 不死，下次 applyPagePlacement 重新挂接即恢复）；④FemoEditorPage = 编辑器页（key=sessionId 保证切 Session 全量重载），editor-view 的全部逻辑迁入（session-state 加载/409 冲突弹窗/定稿落盘/导出导入/preflight/restore 横幅/run_result 回传/triggerRun）。
...
## 2026-08-27 register legacy 'femo-plugin/turn-scope' for session-event registry
### root cause
src/index.ts only registered 'femo-plugin/chat' via registerSessionEventType. Legacy
sessions (pre turn-scope-file refactor) still contain 'femo-plugin/turn-scope' log
...
"femo-plugin/turn-scope" unknown to this harness and not marked ignorable).
### change
src/index.ts: register 'femo-plugin/turn-scope' alongside 'femo-plugin/chat'.
### verification
...
尚无 turn/start + 进程内 Set）：从 god 窗复制可判定为戏内的事件——①femo-plugin/chat 行
...
来自 /femo-plugin/actors 即剧本记录，chatActors 兜底；没跑过剧本两者皆空 → 菜单只有
...
【rAF+30ms】→ handleRunWorkflow → POST /femo-plugin/run。rAF 在隐藏窗口/后台标签
...
### 不动项（有意保留）
- host 侧 POST /femo-plugin/create-session 路由保留（编程式新建 Femo 会话入口；前端调用点已随按钮清零，lib/client.js 该字面量=0）。
...
### 验证
node build.mjs 双产物 exit 0；lib 字面量：host tool_result/mirrorBuffer/BUFFERED_CHILD_EVENTS 全在、client kind:\"toolresult\"/femo-stream-toolresult/retain 全在；tsc host 0 错；tsc client 14 条=历史基线零新增。3081 已重启（node pid 15592 CreationDate 07:57:54，/ 200，/femo-plugin/actors 200）。待猫猫跑带工具的 par 剧本实测：直播期名字+桶+Deep diving 稳定钉在骨架旁、turn 落地成官方节点连续区块、名字贴段头。
...
3. src/subagent.ts：签名加 nodeShowprompts 参数；appendShowpromptLine()（幂等 showpromptWritten，落 femo-plugin/chat {kind:'prompt', text:'📢 '+showprompt, turn:baseTurn, visible:scopeInfo}），调用点=appendSpeakerLine 旁（首个将落盘镜像事件之前）+ finally 兜底（与 speaker 兜底完全同款，带 turn 风险等价）。
...
### 需求（猫猫原话）
"帮我改femo-plugin的工具系统提示词吧，给节点/子agent注入的里面是不是有一句，说如果你看不到上下文你应该用什么工具……并介绍了个工具，那个？把那个删了。"
### 实现（src/subagent.ts）
...
### 需求（猫猫原话）
主模型经"上下文注入 femo-plugin"下场参与戏内节点时，这一整轮发言应注入戏内窗与角色窗："注入的显示依然和以前一样，节点showprompt、角色标签、整轮（包括cot，response，tool和多轮react直到结束），所有这些全都按顺序，不要乱，要放在一起，不要和其他节点交错显示，要流式输出"；"我强烈建议你想办法复用我们之前的投影方法，已经成功过的是不是比较稳"；**"再次强调，god窗现在是对的，千万别动。主模型主窗口也是对的，别动。"** 角色窗按节点 scope 分发（scope 包含谁注入给谁的视角窗）。
### 现状链路（重读确认）
...
5. **src/index.ts**：import 改 apiRetry 单例+broker；RunState 字面量 +giveups:new Map()；registerEngineEventHandlers deps 传 broker；删本地 new ApiRetryChain()；bridge.onExited +broker.abortAll('bridge exited')（停靠者不干等 15min）；+ctx.effect(broker.dispose, 'femo-plugin: node retry broker')（HMR 清 parker/timer）。
...
flow_stopped case 在 appendChatBroadcast 之后补 steerMainAgent：文案 "[femo-plugin] 剧本运行结果：⏹ 已停止（挂起，断点保留）。可用 resume 续跑或 fresh_start 重跑。"，注释注明时序依据（suspended 落定+activeJobId 清空后 isSessionRunning 已 false，不被 pre-step 门卫吞，与 flow_done/flow_error 同时序）。
### 不影响别处论证
...
### 验证
node build.mjs 双产物 exit 0；lib/index.js L3552 steer 字面量在位（紧跟 L3551 广播，中文 \uXXXX 转义形态核验）。**次级坑记录在案未动手**：job_manager.py L352 引擎窗口已死（B5）时停止走"直接落盘 suspended(reason=user_stop)"路径、不发 flow_stopped 事件——该边角连广播都没有，待议是否在宿主 stop 命令层补通知。待重启 3081 后猫猫实测：跑一本中按停止→主会话应出现 [femo-plugin] 停止通知。
...
4. **src/client.tsx**：femo-plugin-count 座位 + lineage fork shadow 两处 slots 注册整体挪入 registerLegacyCatalogUi()，仅 native-flag=legacy（或 fetch 失败 fail-open）时执行——0.1.3 原生目录完全交还本体官方组件（样式正确、femo-node 条目天然可见）。fork 内 femoNativeCatalog 剥前缀逻辑保留未删（旧版专用）。
...
2. **src/routes.ts**：新增 `/femo-plugin/current-job?sessionId=`（返回 sidIndex 最近一场 Job id，跨重启可用）；/projection-windows 原生版角色项改按当前 Job 解析（与菜单同口径）。
...
### 不影响别处论证
旧版路径零改动（lineage-fork.jsx/femo-plugin-count 座位照旧）；同 Job 内复用、冷恢复、interrupt、停靠重试、镜像 V6 时序全部不变——只是身份键从 (sid,actor) 升级为 (sid,job,actor)。resume 语义天然正确：同 jobId 续跑=同 childId=原窗口接续。
### 验证
...
3. **src/routes.ts**：/projection-windows 删原生分流——角色项统一从 windows.actors（femo-proj 窗 id）解析，重启恢复走 turn_scopes 文件 ensure（新旧同款）；删除 /femo-plugin/current-job 路由（唯一消费方已下线）；删 nativeActorChildIds import。
...
2. **戏外缺 sys 行=windowing-native 无条件降级过于保守**：原生分支一律不写主会话。但本实验室 3083 构建（vanilla 检出）的 KNOWN_SESSION_EVENT_TYPES 已打 'femo-plugin/chat' 补丁（源码+lib 双验，注释自证"whitelisted so femo chat history survives restarts"）——持久层安全，降级不再必要。
...
2. **src/windowing-native.ts**：新增 mainChatSafe=安装时运行时探测 KNOWN_SESSION_EVENT_TYPES.has('femo-plugin/chat')（读宿主同实例 dsh-session，探测的是部署真实持久层词表，非编译期假设）；broadcastCompat/projectedCompat 的原生分支从"一律不写主会话"改为"白名单在=旧版同款主会话+全窗双写 / 不在=仅窗侧降级"。官方 stock 无补丁构建自动落回保守行为；meow fork legacy 直通不受影响。
### 不影响别处论证
...
### 验证
纯净 lib（无 femo-plugin/chat 字符串）+ 重启：启动日志 runtime whitelist registered (host copy: ...packages/core/session/lib/index.js) → projection windows durable + main-session allowed；/projection-windows 正常返回 9 角色窗；listChildren 12 行含 9 proj。tsc 80 行=基线-2。
...
2. **src/client.tsx（openSession 原生分支）**：openSubagent 重试链之前先 `await fetch('/femo-plugin/projection-windows?sessionId=主会话')`——宿主 ensure 幂等且完成后窗已唤醒+镜像并集进 live 会话，客户端 history 首拉直接命中全量内容（官方 history 为 live-preferred）。
3. **src/routes.ts**：新增临时诊断路由 `/femo-plugin/debug-live-events`（读宿主 live 会话 eventCount/seq/inheritedEventCount，排障用，随排障面一起摘除）。
### 实测验证（3083 重启后）
...
### 定性与修复
1. **layout 报错 = femo-plugin 缺 inject 声明**：client.tsx 的 `inject=['slots','sessions']` 未声明 'layout'，而 scriptViewInjected 的 toggleSidebar（编辑器「返回壳层」按钮，editor-page onBackToShell）访问 `ctx.layout.toggleSidebar()`——0.1.3 cordis 对未声明服务直接抛该错（堆栈指向宿主 index bundle 的 ctx getter，故文件名是 index-QCwDBYTX.js 而非 femo client.js）。修复：inject 补 'layout'（0.1.3 ILayout.toggleSidebar 存在，API 兼容）。meow-smooth 自身已声明 layout，排除。
2. **femoGen 运行/停止：宿主侧全链路正常**。宿主日志实锤：job 742 于 23:28:40 flow_done（运行成功跑完）；随后多次 stop(explicit) 全部 `stopped=true state=finished` 幂等应答。「没反应」是 femoGen 客户端 UI 不刷新（其 FoldDock composer.dock 槽 0.1.3 适配崩溃已被 SlotErrorBoundary 隔离，运行状态无从展示），「闪退」即该 UI 崩溃面。属 meow-smooth（femoGen）插件自身的 0.1.3 适配问题，与 femo-plugin 后端无关，另行立项。
### 验证
...
### 用户需求（原话）
"帮我改一下 femo-plugin 这个插件吧。给我在 FEMOGen 的设置面板调试窗口右上角加个清空按钮。"
### 背景（为什么只是"补回来"）
...
- **产物**：`npm run build` 通过（lib/client.js 798.6kb，含 `onClick: onClear` 与新按钮）。dsh-plugins/femo-plugin 是指向本仓库的软链，刷新 dsh 页面即生效（纯前端改动，无需重启）。
...
### 用户需求（原话）
"帮我改一下femo-plugin这个插件，在femogen右上角的按钮里，现在当剧本暂停时，右上角是继续和重新开始。当前俩按钮的逻辑有点别扭。
...
- **产物**：`npm run build`（node build.mjs）通过 —— lib/client.js 817.0kb；dsh-plugins/femo-plugin 是指向本仓库的软链，刷新 dsh 页面即生效（纯前端改动，无需重启）。
...
- **产物**：`npm run build`（插件）→ lib/client.js 816.1kb；`npm run build`（femoGen）→ dist/assets/main-DFQkYfOh.js 414.64kb。dist 压缩产物复核：手机三枚 `(idle||paused)&&onRun(s)` / `running&&onStop(a)` / `paused&&onResume(u)`，桌面 `reset:!0` + `background:"var(--femo-success)"` + 琥珀 `borderColor:"var(--femo-warning-border)"`；退役串 0。刷新 dsh 页面即生效（dsh-plugins/femo-plugin 是软链）。
### 两个判断点（都不合意的话各改一行）
...
### 现状（为什么会"走马灯"）
1. **重放帧与活帧形状完全一致**：宿主 `/femo-plugin/events` 新连接会重放 `runState.lastEvents`（cap 400，`ai_token/step` 不入环，见 engine-events.ts:159）——为的是"运行中打开编辑器标签页/手机切回"能把画布状态补齐。但重放发出的 SSE 信封与实时事件逐字节相同（routes.ts:814），前端无从分辨。
...
- **重启后核验**：`session-state` 200（running=false jobId=800 script 4217ch 仍在）、`/femo-plugin/job?job_id=800` 200（引擎桥活着，档案 state=finished）、`/femo-plugin/femo-files` 200、`/events` 200 且流头 `: connected`；重放环重启后为空（符合预期）。
...
### 现状（为什么要做）
1. 调试干跑（`femo_debugger.py` FakeHost 替 AI/人类发言、零 token）此前只有一个**人看**入口：femoGen 调试窗头部「编译」按钮 → `POST /femo-plugin/debug-run` → NDJSON 流式 → debugPanel 逐行渲染。主模型自己写完/改完剧本只能拿正式 `femo-run` 当第一遍测试——编译、接线、死循环都要真烧 token 才发现。
...
3. **`src/index.ts`**：注入 `toolDeps.debugRun`——读会话挂载剧本（`readSessionScriptText` text 优先=实际运行版本；`prev.path` 供 `code: file:` 相对引用解析，与正式运行同语义）→ `collectDebugRun`。与 `femo-run` 的关键差别：**不查 Job 守卫、不 check 编译、不开 Job、不广播**（调试器自己会编译并原话报错；纯只读干跑），并打一行 `[femo-plugin] femo-debug <sid> → exit/records/report/timedOut（耗时）` 诊断。
...
6. **系统提示词（工具 schema 之外的注入面）**：
   - preset persona `dsh-home/.agent-presets/femo-plugin/agent.cordis.yml`：【运行】段两种工作流都插入干跑自检环节、工具线从「三个专用工具」改为「四个专用工具」（顺手修掉 stale 的 `pause 暂停`——schema 里从来没有 pause 动作）、**新增【干跑自检】整段**（返回什么/拿它确认什么/代价为零/跑的是挂载版/多轮与种子/合成输入不代表内容质量）。
...
- **重启执行**：16:25:27 detached 请求 → 16:25:33 start-meow 停旧实例（pid 12896）→ 新实例 **pid 28720** 起来，`/femo-plugin/models` 200。
- **生效核验**：新实例启动日志里 `[femo-plugin] tool registered: femo-debug` 出现 1 次（六件套齐全：mount/run/debug/script/soul/chronica）——本轮改动正式上线。
- **线上实测**（重启后的 live 宿主，真 python 真调试器）：`POST /femo-plugin/debug-run` → 200 + `application/x-ndjson`，9 条记录（run_start/edge/node_start/ai_reply/assign/flow_outcome/run_end/debug_done），末条 `debug_done(exitCode=0)`——干跑链路在线上端到端可用。
...
2. `src/client-ui/composer.tsx`：投影窗发消息遇 **404** 时，先按主会话 id 打一次 `/femo-plugin/projection-windows`（唤醒宿主，幂等），再重发一次；仍失败才把后端原话上屏（并写留痕）。`session … not found` 直冲用户面前的老路径封死。
...
4. `src/routes.ts` `/femo-plugin/projection-windows`：主会话不在 store 时**不再直接 503**，先 `ensureSessionLive` 把主会话拉活（拉活后 cwd 与子代理目录齐备，`projections.ensure` 才有落点）；只有拉活也失败才 503，且带 `kind:'main-not-loaded'` + 可操作中文提示。`/femo-plugin/projection-input` 的调用点补注入 `ensureMainLive`。
...
- **通道**：`src/diag-feed.ts` 已有三路落点（内存环 800 条 + SSE `femo_diag` + `user_data/debug-diag-feed.log`），`GET /femo-plugin/diag-tail?n=` 可拉历史；SSE 走的就是前端已连的 `/femo-plugin/events`——**通道齐全，且此前无前端消费者**。
### 改动
...
3. **`femoGen/src/FemoWorAuto.jsx`**：`compilerLog` 状态（新在前、上限 300、文本截断 400 与宿主同口径）+ `handleDiagFeed`（只认 `tag==='engine'`）+ SSE `onmessage` 里拦 `evt.type==='femo_diag'`（**不进 handleWorkflowEvent**——那不是画布事件，进去只落 default 分支）+ 开面板时 `GET /femo-plugin/diag-tail?n=400` 拉历史（按 ts+文本去重后拼到实时流后面；独立模式无此路由，故 `plugin` 门控）。
...
1. **编辑器页是单例 keep-alive**：`client-ui/editor-page.tsx` 把 femoGen 单页 createRoot 到 body 级容器，切走只把容器设 `visibility:hidden`（`placement -> hidden`），**不卸载**；日志里那句 `single-instance keep-alive` 即此。故 FemoWorAuto 常驻 → `subscribeFemoLog` 订阅、`connectSse` 的 `/femo-plugin/events` 连接（编译器/Host 两页的实时源）都**保持不断**。
...
- **线上实测**：3081 的 dsh 进程是 09-11 19:12 启动的，而 Host 采集（host-log.ts）20:07 之后才构建——进程里没有这段代码。现场触发一次插件 console.log（GET /femo-plugin/actors），诊断流零 host 行，实锤。
- **结论**：再重启一次 3081，钩子即常驻进程 → Host 页从此实时滚动，日常使用零重启。"Log Collector"已存在（src/host-log.ts），只是那次重启时它还没写出来。
- **部署路径确认**：dsh-home/profiles/node_modules/femo-plugin 与 dsh-plugins/femo-plugin 均为指向本仓库的符号链接，重启必然加载新代码，无需同步拷贝。
...
- 09:36 detached 重启 3081（旧 pid 19264 → 新 pid 2208），本次启动段 [femo-plugin] 六工具注册 + bridge started，脚本验证文件 up=True。
### 生效方式
...
### 改动（src/routes.ts，/femo-plugin/session-state 对账点，宿主侧唯一改动点；引擎零改动）
...
## 2026-09-14 全局换标：🎭 行首图标 → FEMO logo（猫猫拍板：面具图标一律换 femo logo）

### 背景
全目录扫描 🎭（含 literal/\ud83c\udfad/\u{1F3AD}/&#x1F3AD;/&#127917/:performing_arts:/FA masks-theater 全形态）：
- 内容契约层（**不动**）：消息文本的 🎭 前缀是存储契约（AI 上下文/搜索/历史/页签路由键），历史 log 与未来消息照旧带 emoji 存；
- 渲染层（**改**）：dsh 客户端唯一渲染面具图标的地方是 femo 客户端 chat-node.tsx 的 LEADING_ICON_MAP（'🎭'→FaMasksTheater）与 composer 等待条；dsh 本体 bundle 无任何面具图标渲染。

### 改动（client/ 三文件，构建即生效于浏览器刷新）
1. fa-icons.tsx：新增 `FemoLogo` 组件——仓库根 FEMO_logo.svg 原样内联（六边形+三角箭头+圆环，品牌绿 #6FBF3A，viewBox 27.17 30 345.66 364）；SVG mask id 全文档唯一性用 useId 生成（多行聊天同屏重复渲染防 id 冲突）；
2. client-ui/chat-node.tsx：LEADING_ICON_MAP 的 '🎭' 从 FaMasksTheater → FemoLogo（行首 emoji 换图标机制不变，其余 emoji 映射原样）；
3. client-ui/composer.tsx：舞台等待条从 `<LeadingIconText text={"🎭 "+prompt}>` 改为 FemoLogo(12px)+prompt 内联组合（等价渲染，少一层字符串拼接）。

### 验证
- node build.mjs 全量构建过（esbuild 直连 .pnpm 实体路径绕开改名断链亦可，见下）；lib/client.js 内 LEADING_ICON_MAP="🎭": FemoLogo、composer 为 FemoLogo+run.prompt；
- 全目录复扫：运行时产物（user_data/cache/.tmp/femo-zcode-dist）之外，源码中 🎭 全部集中在内容契约层（engine-events/projection/tools/zcodeAdapter 的消息文本与注释），按设计不动。

### 关联事故：目录改名 dsh-femo → femo-plugin 的断链
- 触发：2026-09-14 目录改名（另一窗口操作），pnpm 安装的 junction 全部指向旧绝对路径 D:\myFiles\dsh\dsh-femo\... → node_modules 解析全断（esbuild cannot find package）；
- 影响面：femo-plugin 内 108 条 junction 全断（.pnpm 内部互链+平台二进制）；修复方式=重建 junction（新路径）或 pnpm install 重装；
- 临时绕过（本次构建用）：ESBUILD_BINARY_PATH 直指 .pnpm/@esbuild+win32-x64@0.25.0/.../esbuild.exe，createRequire 直连 .pnpm/esbuild@0.25.0/.../esbuild/lib/main.js；
- 插件挂载：dsh-home/profiles/web/node_modules\femo-plugin junction 已同步改指新路径，cordis.patch.yml 插件 id 已是 femo-plugin，3081 重启后链路完整；
- dsh-plugins\dsh-femo 旧 junction 已消失（新链路不依赖，无需重建）。

### 生效方式
- 纯浏览器 bundle 改动，**刷新页面即生效，无需重启 dsh**。
