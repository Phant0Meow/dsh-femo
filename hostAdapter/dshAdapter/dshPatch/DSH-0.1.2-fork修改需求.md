...
### 为谁 / 为什么
femo-plugin 往 Femo 会话写自定义事件 `femo-plugin/chat`（九种 kind 的剧本对话行），渲染靠官方 ConversationNodeDefinition 匹配事件流。**0.1.2 把读路径守卫收得更紧**：
...
后果：不打此补丁，**任何官方构建（含 3080）都无法加载含 `femo-plugin/chat` 的会话**——包括 Femo 主会话（sys 回执写在那里）。本条目把注册面在 0.1.2 上实现掉，并已同步在官方 Discussions 推进上游化（#4815 请愿 + #4204 消费者实证回复，若上游落地本条目归零）。
 ...
 * @param type - the event type string to admit (for example `'femo-plugin/chat'`).
