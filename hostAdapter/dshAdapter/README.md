...
- **接入层**：`index.ts`（插件入口，注册钩子/路由/工具）、`config.ts`（配置）、
  `http.ts`、`routes.ts`（`/femo-plugin/*` HTTP 路由 + SSE 事件流）、`host-log.ts`
...
- **本目录就是 dsh 插件根**（2026-09-13 插件根下沉）：package.json / lib/ /
  ...
  （`npm run build` 或 `node build.mjs`）。引擎侧（femoCompiler / femoBridges /
  femoGen / femo2host / user_data）在仓库根——config.ts 的 `engineRoot` 负责向上
  两层指回，femoRoot 仍可被 dsh 配置显式覆盖。
- `host.manifest.json` 随插件根本地化（与 zcode 侧 zcode.host.manifest.json 同款）。
