 ...
 * routes.ts 的 /femo-plugin/turn-scopes 读取（导出共用实例）。 */
...
export async function runAiSubagent(
  ...
  console.log(`[femo-plugin] ai_request node=${String(request.node_name ?? '')} scope=${String(request.scope ?? '')} soul_id=${soulId}`
  ...
  const onChildEvent = (watched: Session, watchedEvent: SessionEvent): void => {
    ...
    // ①先落带 turn 归属的 femo-plugin/chat error 行（femo-turn-head 渲染在该
