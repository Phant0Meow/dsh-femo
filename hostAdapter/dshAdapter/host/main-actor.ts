...
function mainRetrySteerText(
  ...
  return `[femo-plugin·节点重试] 剧本《${play}》节点「${nodeName}」（你扮演 ${actor}）的上一轮输出未通过剧本校验（第 ${attempt} 次反馈）：\n${feedback}\n请重新输出该节点的台词（${SET_VARIABLE_TEACHING}）。`
}
...
function stageNotice(sid: string, nodeName: string, actor: string, prompt: string, delta: FlowLine[], final: boolean, context = ''): string {
  ...
  const parts = final
    ? [`[femo-plugin·剧终补遗] 剧本《${play ?? '剧本'}》已跑完，以下是最后一截你可见的剧中发言。`]
    : [`[femo-plugin·上场] 剧本《${play ?? '剧本'}》进行到节点「${nodeName}」，轮到 ${actor} 说话。`]
