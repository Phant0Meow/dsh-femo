 ...
 *    ⇒ 主会话不能写 femo-plugin/chat（写入后下次冷恢复即拒载，且 id 被
...
const FEMO_CHAT_TYPE = 'femo-plugin/chat'
...
const LEGACY_FEMO_EVENT_TYPES: readonly string[] = ['femo-plugin/turn-scope']
...
/** 原生模式下主会话写 femo-plugin/chat 是否安全（白名单已含本插件事件）。 */
...
/** 【0.1.3 运行时白名单注册（替代文件补丁，升级免重打）】把 femo-plugin/chat
  ...
  for (const load of candidates) {
    try {
      ...
      if (set !== undefined && typeof set.add === 'function') {
        ...
        console.log(`[femo-plugin][native] runtime whitelist registration: femo-plugin/chat ${ok ? 'registered' : 'failed'}${hostPath !== undefined ? ` (host copy: ...${hostPath.slice(-60)})` : ' (plugin-local copy)'} (+${LEGACY_FEMO_EVENT_TYPES.length} legacy femo types)`)
        return { ok, hostPath }
      }
    } catch { /* 下一候选 */ }
  }
