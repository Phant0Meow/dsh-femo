 ...
 *
 * 2026-09-13 API 化改造（与 femoCompiler/femo_api.py 同一取向）：宿主接口侧
 * （dshAdapter/client 等）只准 import 本文件，不再深入 femoGen/src 内部——
 * 编辑器内部重构（拆文件/改实现）只要保住本文件的导出与 props 契约，
 * 宿主零改动。
 *
 * ── 宿主消费面全清单（2026-09-13 盘点，此外别无）────────────────────────
 *  ① 源码级 import（唯一一条代码耦合）：编辑器应用组件（下方 re-export）。
 *     消费者 = dshAdapter/client/client-ui/editor-page.tsx——「Femo 编辑器」
 *     单页常驻宿主，把本组件打进聊天窗 bundle（esbuild 从仓库根解析相对
 *     路径，不经过 vite）。
 *  ② dist 文件级约定（不是代码 import——Node 进程解析不了 JSX）：构建产物
 *     目录 femoGen/dist。zcode 网关（zcodeAdapter/mcp/femo-server.mjs）存在
 *     即静态托管；打包清单（scripts/sync-zcode-dist.ps1）按字面路径拷贝。
 *     改目录名 = 三处同步（vite.config 的 outDir + 上述两处）。
 *  ③ 反向 HTTP 契约（femoGen 前端 → 宿主，方向与本文件相反）：/dsh-femo/*
 ...
 *
 * ⚠️ props 语义的权威实现在 src/FemoWorAuto.jsx（组件签名 + 内部消费）；
 * 本 typedef 是插件模式词汇表的门面快照，改签名先改这里再动组件。
 */

export { default, default as FemoEditorApp } from './src/FemoWorAuto'
